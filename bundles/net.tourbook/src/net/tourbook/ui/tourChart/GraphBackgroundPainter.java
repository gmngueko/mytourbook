...
public class GraphBackgroundPainter implements IFillPainter {
   ...
   private void createColors_HrZone(final TourPerson tourPerson) {
      ...
      for (int colorIndex = 0; colorIndex < personHrZones.size(); colorIndex++) {
         ...
         _hrZone_Colors[colorIndex] = new Color(Display.getDefault(), rgb);
      }
   }
   ...
   private void createColors_SwimStyle() {
      ...
      for (final Entry<SwimStroke, RGB> swimStrokeItem : SwimStrokeManager.getSwimStroke_RGB().entrySet()) {
         _strokeStyle_Colors.put(swimStrokeItem.getKey().getValue(), new Color(Display.getDefault(), swimStrokeItem.getValue()));
      }
   }
